...
pub struct FftTempogramResult {
    ...
    ///
    /// Diagnostic peak height retained from upstream and asserted by this
    /// module's tests; Sustain's tempogram consumer reads only `bpm` and
    /// `confidence`, so it is otherwise unread in non-test builds.
    #[allow(dead_code)]
    pub power: f32,
}
