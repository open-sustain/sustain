...
CREATE TABLE IF NOT EXISTS tracks (
    ...
    -- "Sort as" names (issue #13). Captured from file tags at import or
    -- generated by Sustain for leading articles, used for ordering, and
    -- preserved by full metadata mirrors where the active backend can
    -- round-trip the standard tag key. NULL when no alternate sort value is
    -- needed.
    title_sort TEXT,
    artist_sort TEXT,
    album_sort TEXT,
...
SET
    title_sort = CASE
        WHEN title_sort IS NULL OR TRIM(title_sort) = '' THEN
            CASE
                WHEN title IS NOT NULL AND LOWER(TRIM(title)) LIKE 'the %' AND TRIM(SUBSTR(TRIM(title), 4)) <> '' THEN TRIM(SUBSTR(TRIM(title), 4)) || ' (The)'
                WHEN title IS NOT NULL AND LOWER(TRIM(title)) LIKE 'a %' AND TRIM(SUBSTR(TRIM(title), 2)) <> '' THEN TRIM(SUBSTR(TRIM(title), 2)) || ' (A)'
                WHEN title IS NOT NULL AND LOWER(TRIM(title)) LIKE 'an %' AND TRIM(SUBSTR(TRIM(title), 3)) <> '' THEN TRIM(SUBSTR(TRIM(title), 3)) || ' (An)'
            END
        ELSE title_sort
    END,
    artist_sort = CASE
        WHEN artist_sort IS NULL OR TRIM(artist_sort) = '' THEN
            CASE
                WHEN artist IS NOT NULL AND LOWER(TRIM(artist)) LIKE 'the %' AND TRIM(SUBSTR(TRIM(artist), 4)) <> '' THEN TRIM(SUBSTR(TRIM(artist), 4)) || ' (The)'
                WHEN artist IS NOT NULL AND LOWER(TRIM(artist)) LIKE 'a %' AND TRIM(SUBSTR(TRIM(artist), 2)) <> '' THEN TRIM(SUBSTR(TRIM(artist), 2)) || ' (A)'
                WHEN artist IS NOT NULL AND LOWER(TRIM(artist)) LIKE 'an %' AND TRIM(SUBSTR(TRIM(artist), 3)) <> '' THEN TRIM(SUBSTR(TRIM(artist), 3)) || ' (An)'
            END
        ELSE artist_sort
    END,
    album_sort = CASE
        WHEN album_sort IS NULL OR TRIM(album_sort) = '' THEN
            CASE
                WHEN album IS NOT NULL AND LOWER(TRIM(album)) LIKE 'the %' AND TRIM(SUBSTR(TRIM(album), 4)) <> '' THEN TRIM(SUBSTR(TRIM(album), 4)) || ' (The)'
                WHEN album IS NOT NULL AND LOWER(TRIM(album)) LIKE 'a %' AND TRIM(SUBSTR(TRIM(album), 2)) <> '' THEN TRIM(SUBSTR(TRIM(album), 2)) || ' (A)'
                WHEN album IS NOT NULL AND LOWER(TRIM(album)) LIKE 'an %' AND TRIM(SUBSTR(TRIM(album), 3)) <> '' THEN TRIM(SUBSTR(TRIM(album), 3)) || ' (An)'
            END
        ELSE album_sort
    END,
    album_artist_sort = CASE
        WHEN album_artist_sort IS NULL OR TRIM(album_artist_sort) = '' THEN
            CASE
                WHEN album_artist IS NOT NULL AND LOWER(TRIM(album_artist)) LIKE 'the %' AND TRIM(SUBSTR(TRIM(album_artist), 4)) <> '' THEN TRIM(SUBSTR(TRIM(album_artist), 4)) || ' (The)'
                WHEN album_artist IS NOT NULL AND LOWER(TRIM(album_artist)) LIKE 'a %' AND TRIM(SUBSTR(TRIM(album_artist), 2)) <> '' THEN TRIM(SUBSTR(TRIM(album_artist), 2)) || ' (A)'
                WHEN album_artist IS NOT NULL AND LOWER(TRIM(album_artist)) LIKE 'an %' AND TRIM(SUBSTR(TRIM(album_artist), 3)) <> '' THEN TRIM(SUBSTR(TRIM(album_artist), 3)) || ' (An)'
            END
        ELSE album_artist_sort
    END,
    composer_sort = CASE
        WHEN composer_sort IS NULL OR TRIM(composer_sort) = '' THEN
            CASE
                WHEN composer IS NOT NULL AND LOWER(TRIM(composer)) LIKE 'the %' AND TRIM(SUBSTR(TRIM(composer), 4)) <> '' THEN TRIM(SUBSTR(TRIM(composer), 4)) || ' (The)'
                WHEN composer IS NOT NULL AND LOWER(TRIM(composer)) LIKE 'a %' AND TRIM(SUBSTR(TRIM(composer), 2)) <> '' THEN TRIM(SUBSTR(TRIM(composer), 2)) || ' (A)'
                WHEN composer IS NOT NULL AND LOWER(TRIM(composer)) LIKE 'an %' AND TRIM(SUBSTR(TRIM(composer), 3)) <> '' THEN TRIM(SUBSTR(TRIM(composer), 3)) || ' (An)'
            END
        ELSE composer_sort
    END;
"#;
