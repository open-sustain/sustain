...
//! seek-and-patch passes. The `columns` table is a fixed schema, identical
//! on every export, and is generated from the shared machinery (see
//! `write_columns_page`). The three `history` tables are structurally
//! load-bearing — the hardware refuses a drive whose history tables are
//! absent — but the reference pages also carry the maintainer's captured
//! session data and use a row-group footer with the two trailing fields in
//! the opposite order from the data tables, so they remain validated
//! reference blobs pending the derived-content work tracked in #166.
...
// Structurally required history reference pages. Unlike `columns` (a fixed
// schema, now generated by `write_columns_page`), these carry captured
// session data and a variant row-group layout; they stay verbatim until
// the derived-content work in #166 lands and can be hardware-validated.
...
/// table and its rows use the [`RowGroupUnknown::Identity`] footer the
...
mod tests {
    ...
    #[test]
    fn history_reference_pages_are_embedded() {
        ...
        let bytes = build(&tracks, &playlists, &[], "2026-01-01").expect("build pdb");
        // The three history pages are still carried verbatim (#166).
        assert_eq!(
            &bytes[36 * PAGE_SIZE..37 * PAGE_SIZE],
            REF_HISTORY_PLAYLISTS
        );
        assert_eq!(&bytes[38 * PAGE_SIZE..39 * PAGE_SIZE], REF_HISTORY_ENTRIES);
        // History page is the blob with its DB-sequence stamp patched at 0x10.
        let hist = &bytes[40 * PAGE_SIZE..41 * PAGE_SIZE];
        assert_eq!(
            u32::from_le_bytes([hist[0x10], hist[0x11], hist[0x12], hist[0x13]]),
            10
        );
        let mut expected = *REF_HISTORY;
        expected[0x10..0x14].copy_from_slice(&10u32.to_le_bytes());
