...
//! - `i18n-check` — gate the localization contract (template/catalog drift,
//!   header integrity, named-placeholder rule, fuzzy/untranslated/obsolete
//!   entries, missed call shapes).
//! - `i18n-compile` — build the installable `.mo`, desktop, and AppStream
//!   artifacts under `target/i18n/dist/`.
//!
//! The command implementations take an explicit workspace root (`run_in`), so
//! the `tests/` suite can drive them against fixture workspaces.
...
/// Parse the command line and run the requested command, returning the process
/// exit code.
pub fn run() -> ExitCode {
